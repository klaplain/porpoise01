...
uint16_t filetransferbuffer[2048];
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	// Fake a date time of 9:20:10 on 6/22/2025

	SPI_buffer[1]=9; // set hours
	SPI_buffer[2]=20; // set minutes
	SPI_buffer[3]=10; // set seconds
	SPI_buffer[5]= 6; //month
	SPI_buffer[6]=22; // date
	SPI_buffer[7]=25; // year



	datetime_request_handler();  // Set the RTC

	//directory_request_handler(); // read the SSD

	// Fake a recording
	SPI_buffer[1]=9; // set sampling freq
	SPI_buffer[2]=20; // set sampling freq
	SPI_buffer[3]=1; // set transducer gain B
	SPI_buffer[4]=6; // set millisecs to record
	SPI_buffer[5]=7; // Set millisecs to record

	recording_request_handler2();  // Let's record some stuff
...
void save_request_handler2(){
	...
	printf("opening file %s to upload2\r\n", filename_buffer);
	if(f_open(&SDFile, filename_buffer, FA_READ) != FR_OK)  				//Open file for reading and uploading
	{
		printf("FAIL: Cannot open file for reading uploading\r\n");
	}
	else
	{
		...
		while(!f_eof(&SDFile)){
			HAL_GPIO_WritePin(GPIOC, GAINA0_Pin, GPIO_PIN_SET);
			result = f_read(&SDFile, &filetransferbuffer[0],4096 , (void *)&bytes_read);
			HAL_GPIO_WritePin(GPIOC, GAINA0_Pin, GPIO_PIN_RESET);
			if((bytes_read == 0) || (result != FR_OK))
			{
				printf("FAIL: Cannot read file to save upload\r\n");
			}
			else
			{
				...
				HAL_GPIO_WritePin(GPIOC, GAINA1_Pin, GPIO_PIN_SET);
				HAL_GPIO_WritePin(Acq_Busy_GPIO_Port, Acq_Busy_Pin, GPIO_PIN_RESET);  // Tell RASPI we are Ready
				HAL_SPI_Transmit(&hspi6, (uint8_t *)filetransferbuffer[0], 4096, 5000);
				HAL_GPIO_WritePin(Acq_Busy_GPIO_Port, Acq_Busy_Pin, GPIO_PIN_SET);  // Tell RASPI we are Busy
				HAL_GPIO_WritePin(GPIOC, GAINA1_Pin, GPIO_PIN_RESET);
...
/*	// Now read the number of bytes needed for the filename
	...
	printf("Record: Sampling %3dkHz  Gain %1d   Duration %lumS   \r\n", sampling_frequency_kHz, SPI_buffer[3], millisecs_to_record);
